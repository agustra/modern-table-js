...
export class ModernTable extends EventEmitter {
    constructor(selector, options = {}) {
        ...
        if (this.options.keyboard && this.options.buttons) {
            const hasShortcuts = this.options.buttons.some(btn => 
                btn === 'shortcuts' || 
                (typeof btn === 'object' && (btn.extend === 'shortcuts' || btn.text?.includes('keyboard')))
            );
            if (!hasShortcuts) {
    ...
    createToolbar() {
        ...
        const hasFeatures = this.options.searching || 
    ...
    createSearchBox(container) {
        ...
        this.searchInput = createElement('input', {
            ...
            placeholder: 'Search...',
            style: 'width: 250px; padding-right: 30px;'
        });
        ...
        const clearBtn = createElement('button', {
            ...
            innerHTML: '&times;',
            style: 'right: 5px; top: 50%; transform: translateY(-50%); border: none; background: none; display: none; z-index: 10;'
        });
    ...
    createPagination() {
        ...
        this.paginationContainer = createElement('div', {
            className: 'modern-table-pagination d-flex justify-content-between align-items-center mt-3'
        });
        ...
        this.options.columns.forEach(column => {
            ...
            if (column.render) {
                if (typeof column.render === 'function') {
                    cellValue = column.render(cellValue, 'display', rowData, { row: index, col: 0 });
    ...
    toggleColumnVisibility() {
        ...
        const colvisBtn = find('button[data-action="colvis"]', this.toolbar) || 
                         findAll('button', this.toolbar).find(btn => btn.textContent.includes('Columns'));
    ...
    applyAllColumnVisibility() {
        ...
        Object.keys(this.columnVisibility).forEach(columnIndex => {
            ...
            if (visible === false) { // Only apply hidden columns
                ...
                <table>
                    ...
                    <tbody>
                        ${data.map(row => `
                            <tr>
                                ${columns.map(col => {
                                    let cellValue = this.getCellValue(row, col.data);
                                    
                                    // Apply column render function for print
                                    if (col.render && typeof col.render === 'function') {
                                        cellValue = col.render(cellValue, 'print', row, {});
                                    }
...
• Use Shift+Arrow for multi-select`;
        ...
        popup.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                ...
                <button style="border: none; background: none; font-size: 18px; cursor: pointer; padding: 0; width: 24px; height: 24px;">×</button>
            </div>
            <div style="font-size: 13px; color: #666;">${shortcuts}</div>
        `;
